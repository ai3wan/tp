...
def get_step_keyboard(step: int) -> ReplyKeyboardMarkup:
    ...
    keyboards = {
        ...
        6: ReplyKeyboardMarkup(
            keyboard=[
                [KeyboardButton(text="✊ Кулаки расслабил")],
                [KeyboardButton(text="🏠 В основное меню")]
            ],
            resize_keyboard=True
        ),
        7: ReplyKeyboardMarkup(
            keyboard=[
                [KeyboardButton(text="🎒 Сбросил напряжение")],
                [KeyboardButton(text="🏠 В основное меню")]
            ],
            resize_keyboard=True
        ),
        8: ReplyKeyboardMarkup(
            keyboard=[
                [KeyboardButton(text="🙂 Расслабил лицо")],
                [KeyboardButton(text="🏠 В основное меню")]
            ],
            resize_keyboard=True
        ),
...
async def start_day_2_module_1(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "<b>📝 Теория:</b>\n"
        "Когда мозг включает режим тревоги, активируется симпатическая нервная система. Она как «педаль газа» — сердце начинает биться чаще, дыхание ускоряется, мышцы напрягаются, готовясь к действию. Даже если действовать не требуется.\n\n"
        ...
        "Поэтому тревога ощущается телесно: затекшие плечи, сжатая челюсть, дрожь, холодок в животе. Это не болезнь, а естественная реакция организма, только немного неуместная в повседневности.",
        parse_mode="HTML",
...
async def step_1_to_2(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "<b>📝 Теория:</b>\n"
        "Это связано с амигдалой — маленькой частью мозга, отвечающей за страх. Она работает быстрее сознания. Это как пожарная сигнализация: сначала орёт, а потом уже можно разбираться, где возгорание.\n\n"
        ...
        "Такой порядок — эволюционная защита. Если бы древний человек сначала думал, а потом напрягал мышцы, он не успел бы убежать. Сегодня этот механизм часто мешает, потому что сигнал включается без реальной угрозы.",
        parse_mode="HTML",
...
async def step_2_to_3(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "<b>📝 Теория:</b>\n"
        "Это называется соматизацией — тревога выражается через телесные ощущения. Человек может не осознавать волнение, но тело хранит зажимы.\n\n"
        "Самые «любимые» зоны тревоги — плечи и шея. Они поднимаются, словно защищая шею. Часто напрягаются челюсти, которые «сжимают зубы» от стресса. А ещё живот, где появляется тяжесть или спазм.\n\n"
        "Хорошая новость в том, что, расслабив эти зоны, можно отправить в мозг сигнал: «опасности нет». Связь идёт в обе стороны — напряжение усиливает тревогу, а расслабление её гасит.",
        parse_mode="HTML",
...
async def step_3_to_4(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "<b>📝 Теория:</b>\n"
        "Так формируется порочный круг: тревожная мысль вызывает напряжение в плечах → тело ощущает дискомфорт → мозг думает «тревога усиливается». И так по кругу.\n\n"
        ...
        "Исследования показывают: телесные практики напрямую влияют на мозг. Даже простое расслабление челюстей или несколько медленных выдохов способны снизить активность амигдалы.",
        parse_mode="HTML",
...
async def step_4_to_5(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "<b>📝 Теория:</b>\n"
        "Её разработал врач Эдмунд Джекобсон ещё в 1920-х. Он заметил: поочерёдное напряжение и расслабление мышц снижает уровень тревожности. Тело словно учится отпускать зажимы.\n\n"
        ...
        "Мозг фиксирует: «мышцы мягкие — опасности нет». Со временем этот паттерн закрепляется, и организм переключается быстрее.",
        parse_mode="HTML",
...
async def step_5_to_6(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "Начнём с простого: сжатия и расслабления кулаков.\n\n"
        "<b>📝 Теория:</b>\n"
        "Сожми кулаки как можно сильнее, почувствуй напряжение. Подержи пару секунд. Теперь отпусти. Заметь разницу.\n\n"
        "Когда мышцы отпускают напряжение, в мозг идёт сигнал: «опасности нет». Это маленький жест, но он снижает общий уровень тревоги.\n\n"
        "Чем ярче ощущается переход «напряжено → расслаблено», тем легче телу запомнить новый навык.",
        parse_mode="HTML",
        reply_markup=get_step_keyboard(6)
    )

# Шаг 6 -> Шаг 7
@router.message(Day2Module1States.step_6, F.text == "✊ Кулаки расслабил")
async def step_6_to_7(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Теперь перейдём к плечам — одной из самых «тревожных» зон.\n"
        "Поднять их к ушам, сильно напрячь, задержать, а потом отпустить вниз.\n\n"
        "<b>📝 Теория:</b>\n"
        "Плечи — склад эмоций. Они автоматически поднимаются, словно защищают шею. Это древний механизм: спрятать уязвимое место.\n\n"
        "Если напряжение остаётся, к вечеру появляется скованность, тяжесть, иногда головные боли. Тело носит невидимый груз.\n\n"
        "Когда плечи сознательно напрягаются и отпускаются, старый паттерн разрушается. В мозг идёт сигнал «опасности нет», и тревога снижается.",
        parse_mode="HTML",
        reply_markup=get_step_keyboard(7)
    )

# Шаг 7 -> Шаг 8
@router.message(Day2Module1States.step_7, F.text == "🎒 Сбросил напряжение")
async def step_7_to_8(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Ещё одна зона — лицо.\n"
        "Сжать челюсти, нахмурить брови, а потом отпустить и расслабить всё лицо.\n\n"
        "<b>📝 Теория:</b>\n"
        "Мышцы лица связаны с нервной системой. Улыбка, нахмуренные брови, мягкие губы — всё это сигналы мозгу о состоянии.\n\n"
        "При тревоге челюсти напрягаются настолько, что иногда зубы стискиваются во сне. Лицо становится жёстким, как маска, усиливая тревогу.\n\n"
        "Когда лицо расслабляется — брови разглаживаются, челюсти мягкие — мозг получает обратный сигнал: «всё в порядке». Это снижает тревожность.",
        parse_mode="HTML",
        reply_markup=get_step_keyboard(8)
    )

# Шаг 8 -> Шаг 9
@router.message(Day2Module1States.step_8, F.text == "🙂 Расслабил лицо")
async def step_8_to_9(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "<b>📝 Теория:</b>\n"
        "Прогрессивная релаксация работает как переключатель. Мышцы напрягаются и отпускаются, нервная система фиксирует контраст и быстрее переключается в режим покоя.\n\n"
        "Это репетиция: «опасность — безопасность». Чем чаще повторяется цикл, тем быстрее тело учится включать режим спокойствия.\n\n"
        "Со временем тревога уходит быстрее, а тело реже застревает в напряжении.",
        parse_mode="HTML",
...
async def step_9_to_10(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "<b>📝 Теория:</b>\n"
        "Перед сном дома, во время паузы на работе, в транспорте — никаких условий не нужно, только внимание.\n\n"
        ...
        "Так формируется новый шаблон: вместо привычного зажима в ответ на тревогу возникает привычка расслабляться.",
        parse_mode="HTML",
...
async def step_10_to_11(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "<b>📝 Теория:</b>\n"
        "Тревога — часть биологии. Но навык регулировки работает как тренировка в спортзале: каждое упражнение укрепляет «мышцу спокойствия».\n\n"
        "Сначала результат заметен слегка, но с регулярной практикой привычка становится устойчивой.\n\n"
        "Лучше по 2 минуты ежедневно, чем долго и редко. Постепенность эффективнее силы.",
        parse_mode="HTML",
...
async def step_11_to_12(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "<b>📝 Теория:</b>\n"
        "Теперь есть два приёма: дыхание и работа с мышцами. Это как маленькая «аптечка спокойствия».\n\n"
        ...
        "Каждый такой шаг — вклад в привычку спокойствия. Формируется личный набор техник, которыми можно пользоваться в любых обстоятельствах.",
        parse_mode="HTML",
