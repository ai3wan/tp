...
async def step_5_to_6(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "<blockquote expandable>Сожми кулаки как можно сильнее, почувствуй напряжение. Подержи пару секунд. Теперь отпусти. Заметь разницу.\n\n"
        "Когда мышцы отпускают напряжение, в мозг идёт сигнал: «опасности нет». Это маленький жест, но он снижает общий уровень тревоги.\n\n"
        "Чем ярче ощущается переход «напряжено → расслаблено», тем легче телу запомнить новый навык.</blockquote>",
        parse_mode="HTML",
...
async def step_6_to_7(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "<blockquote expandable>Плечи — склад эмоций. Они автоматически поднимаются, словно защищают шею. Это древний механизм: спрятать уязвимое место.\n\n"
        "Если напряжение остаётся, к вечеру появляется скованность, тяжесть, иногда головные боли. Тело носит невидимый груз.\n\n"
        "Когда плечи сознательно напрягаются и отпускаются, старый паттерн разрушается. В мозг идёт сигнал «опасности нет», и тревога снижается.</blockquote>",
        parse_mode="HTML",
...
async def step_7_to_8(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "<blockquote expandable>Мышцы лица связаны с нервной системой. Улыбка, нахмуренные брови, мягкие губы — всё это сигналы мозгу о состоянии.\n\n"
        ...
        "Когда лицо расслабляется — брови разглаживаются, челюсти мягкие — мозг получает обратный сигнал: «всё в порядке». Это снижает тревожность.</blockquote>",
        parse_mode="HTML",
