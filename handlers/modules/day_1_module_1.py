...
async def start_day_1_module_1(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Привет 👋 Сегодня мы начинаем наш курс. Первое, что важно понять: тревога — это часть жизни каждого человека. Она бывает у всех, даже у самых уверенных людей. Готов узнать, как с ней обращаться?",
...
async def step_1_to_2(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Звучит странно, но тревога — не враг 😌 Она работает как сигнализация: громкая, навязчивая, но с одной целью — предупредить. Иногда эта «сигналка» срабатывает даже тогда, когда опасности нет.",
...
async def step_2_to_3(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Наш мозг создан, чтобы защищать нас 🧠 Он оценивает мир вокруг и ищет угрозы. Но иногда он видит опасность там, где её на самом деле нет — например, перед выступлением или важным разговором.",
...
async def step_3_to_4(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Представь ситуацию: завтра собеседование. Всё идёт нормально, но сердце начинает биться быстрее ❤️‍🔥 ладони потеют, мысли скачут. Это не потому, что что-то плохое случилось — так тело готовит тебя к «битве».",
...
async def step_4_to_5(message: Message, state: FSMContext):
    ...
    await message.answer(
        "И вот главный момент 👉 тревога — это не признак слабости. Это просто сигнал: «Внимание, может быть что-то важное». Она пытается мобилизовать силы, даже если ситуация не такая уж и опасная.",
...
async def step_5_to_6(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Проблема в том, что тревога часто перебарщивает 🙃 Она включает режим «всё горит», когда достаточно было бы маленькой лампочки. Поэтому важно учиться регулировать её громкость.",
...
async def step_6_to_7(message: Message, state: FSMContext):
    ...
    await message.answer(
        "И это хорошая новость 😊 Мы можем научиться дружить с тревогой. Не подавлять её, не гнать прочь, а относиться как к сигналу, который можно приглушить и направить в нужное русло.",
...
async def step_7_to_8(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Первый способ регулировать тревогу — дыхание 🌬️ Это самый простой инструмент, который всегда с тобой. Через дыхание мы можем успокоить нервную систему и вернуть контроль над состоянием.",
...
async def step_8_to_9(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Заметь: когда ты тревожишься, дыхание становится быстрым и поверхностным. Это запускает цепочку «больше кислорода → быстрее сердце → сильнее тревога». Если дыхание замедлить, тревога снижается 🔇",
...
async def step_9_to_10(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Попробуем прямо сейчас 👌 Сделай вдох на 4 секунды, а выдох — на 6. Повтори два круга. Считай в уме или по пальцам — это помогает держать ритм.",
...
async def step_10_to_11(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Хорошо 👏 Обрати внимание: после этих нескольких дыханий тело чуть расслабляется, мысли становятся спокойнее. Это простой, но мощный способ «снизить громкость» тревоги.",
