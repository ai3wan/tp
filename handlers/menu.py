...
@router.message(F.text == "✅ Да, давай пройдем пульс тревожности")
async def start_test_from_onboarding(message: Message, state: FSMContext):
    """Пользователь согласился пройти пульс тревожности после онбординга."""
...
async def show_course_info(message: Message):
    ...
    await message.answer(
        ...
        "• **Персональный подход** — пульс тревожности поможет подстроить курс под тебя\n"
        ...
        reply_markup=ReplyKeyboardMarkup(keyboard=[
            ...
            [KeyboardButton(text="🚀 Начать курс без пульса тревожности")]
        ], resize_keyboard=True)
    )

@router.message(F.text == "🚀 Начать курс без пульса тревожности")
async def start_course_without_test(message: Message):
    """Пользователь хочет начать курс без прохождения пульса тревожности."""
    # Устанавливаем курс тревожности (ID = 1) как активный
    await db.update_user_bookmark(message.from_user.id, 1, 1, 1)
...
async def select_module_entry(message: Message):
    ...
    # Проверяем пульс для курса тревожности (ID = 1), независимо от текущей закладки
    ...
    if not initial_assessment:
        await message.answer(
            ...
            reply_markup=ReplyKeyboardMarkup(keyboard=[
                [KeyboardButton(text="✅ Да, пройти пульс тревожности")],
