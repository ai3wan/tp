...
async def onboarding_final(message: Message, state: FSMContext):
    ...
    image_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "assets", "pulse.jpg")
    
    if os.path.exists(image_path):
        image_file = FSInputFile(image_path)
        await message.answer_photo(
            photo=image_file,
            caption="💬 Знаешь, все эти состояния кажутся сложными, но они естественны — такое испытывает очень много людей...\n\n"
